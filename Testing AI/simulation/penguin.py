...
class Penguin(object):
    ...
    def __init__(self, game_map):
        ...
        self.surface = pg.image.load('penguin.png')
    ...
    def draw(self, screen):
        """Renders the penguin intro the screen"""
        screen.blit(self.rotate_surface, [self.x_pos, self.y_pos])
        self.draw_radar(screen)

    def update(self):
        """Updates the penguin itself"""
        #self.x_pos += dif_x
        #self.y_pos += dif_y
    ...
    def update_radar(self, degree):
        """Updates the penguin radars and appends them to its list"""
        length = 0
    ...
    def check_error(self):
        ...
        try:
            if self.game_map.get_at((
                    int(self.center[0] +
                        math.cos(math.radians(360 - (self.angle))) * 51),
                    int(self.center[1] +
                        math.sin(math.radians(360 - (self.angle))) * 51)
            )) == ERROR_COLOR:
